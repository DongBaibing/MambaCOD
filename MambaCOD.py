...
class MEEM(nn.Module):
    ...
    def forward(self, x):  # 前向传播函数
        x = x.permute(0, 3, 1, 2).contiguous()  # Shape: (B, C, H, W)
...
class cnnBlock(nn.Module):
  def __init__(self,hidden_dim,num_layers):
    ...
    for i_layer in range(num_layers):
      one_layer = nn.Sequential(nn.Conv2d(hidden_dim, hidden_dim, 3, padding='same'), nn.SiLU())
      self.cnn_list.append(one_layer)
  def forward(self,x):
    x = x.permute(0, 3, 1, 2)
    for one_layer in self.cnn_list:
      x = one_layer(x)
    x = x.permute(0, 2, 3, 1)
    return x
...
class fftBlock(nn.Module):
    def __init__(self, hidden_dim):
        super().__init__()
        self.dnn = nn.Linear(hidden_dim, hidden_dim)
    ...
    def forward(self, x):
        x_ori = x
        # print(x_ori.dtype)
        x_fft = torch.fft.fft2(x_ori, dim=(1, 2), norm='ortho').float()
        # x_fft = self.dnn(x_fft)
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.cnn_block = cnnBlock(hidden_dim,1)
        self.fft_block = fftBlock(hidden_dim)

    def forward(self, input: torch.Tensor):
        x_ori = input
        x_cnn = self.cnn_block(input)
        x_fft = self.fft_block(input)
        x_ssd = self.drop_path(self.self_attention(self.ln_1(input)))
        x = input + x_cnn + x_fft + x_ssd
        return x


class VSSBlock_ori(nn.Module):
    def __init__(
        self,
        hidden_dim: int = 0,
        drop_path: float = 0,
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 9, 2], depths_decoder=[2, 9, 2, 2],
        ...
        self.meem_block = MEEM(in_dim=768,hidden_dim=16)
        self.meem_block1 = MEEM(in_dim=96,hidden_dim=16)
